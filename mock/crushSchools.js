/**
 * Minimal school mock for mutual-crush experiment only.
 * Not a national city/school database.
 */
...
function searchCandidates(raw, pendingSchools = []) {
  ...
  if (!isAmbiguousRaw(text)) {
    return { candidates: [], mode: 'pending' }
  }

  const matched = schools.filter((s) => {
    if (text.includes('实验') && s.name.includes('实验')) return true
    if ((text.includes('一中') || text.includes('第一中学')) && s.name.includes('第一')) return true
  ...
  const candidates = unique.slice(0, 2)
  return { candidates, mode: candidates.length ? 'disambiguate' : 'pending' }
}
